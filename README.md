...
```sh
make docker-build docker-push IMG=<some-registry>/platform-service-gitops:tag
```
...
```sh
make deploy IMG=<some-registry>/platform-service-gitops:tag
```
...
```sh
make build-installer IMG=<some-registry>/platform-service-gitops:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/platform-service-gitops/<tag or branch>/dist/install.yaml
```
